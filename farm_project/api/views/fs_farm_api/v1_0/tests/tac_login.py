...
from api.views.fs_farm_api.v1_0 import TacLoginViewSet
...
class TacLoginViewSetTestCase(TestCase):
    ...
    def test_submit_success(self):
        ...
        response = self.client.post(f'/api/v1_0/tac-login/{self.tac.code}/submit/', data=self.valid_request_data, format='json')
    ...
    def test_post_failure(self):
        ...
        response = self.client.post('/api/v1_0/tac-login/', data=self.valid_request_data, format='json')
        self.assertEqual(response.status_code, 501)

    def test_submit_failure(self):
        response = self.client.post(f'/api/v1_0/tac-login/{self.tac.code}/submit/', data=self.invalid_request_data, format='json')
        self.assertEqual(response.status_code, 200)
        json_string = response.content.decode() 
        responseDict = json.loads(json_string) 
    ...
    def test_submit_failure2(self):
        response = self.client.get('/api/v1_0/tac-login/xxx/')
        self.assertEqual(response.status_code, 404)
        
    def test_submit_failure3(self):
        response = self.client.get('/api/v1_0/tac-login/')
        self.assertEqual(response.status_code, 501)

    def test_init_success(self):
        response = self.client.get(f'/api/v1_0/tac-login/{self.tac.code}/init/')
        self.assertEqual(response.status_code, 200)
        json_string = response.content.decode() 
        responseDict = json.loads(json_string) 
    ...
    def test_init_failure(self):
        response = self.client.get('/api/v1_0/tac-login/xxx/init/')
        self.assertEqual(response.status_code, 404)
        
    def test_init_failure2(self):
        response = self.client.get('/api/v1_0/tac-login/init/')
        self.assertEqual(response.status_code, 404)
